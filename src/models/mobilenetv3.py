...
class Model(nn.Module):
    def __init__(self, num_classes):
        ...
        self.base.classifier = nn.Sequential(
            nn.Linear(self.base.classifier[0].in_features, 1280),
            nn.Hardswish(),
            nn.Dropout(p=0.2),
...
def train_model(model, num_epochs=50):
    logger = Logger(log_dir="logs_mobilenetv3", class_names=class_names)
    ...
    torch.save(model.state_dict(), "best_model_mobilenetv3.pt")
    print(f"   -> En iyi model 'best_model_mobilenetv3.pt' olarak kaydedildi.\n")
...
print("\n" + "="*60)
print("TÜM İŞLEMLER BAŞARIYLA TAMAMLANDI (MobileNetV3)")
